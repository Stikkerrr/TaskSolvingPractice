package job4j.dataType;

public class MathAbs {
	public static void main(String[] args) {
		double result = Math.abs(-5);
		System.out.println(result);
	}
}

/* 8. Модуль числа
 *
 * Что если нам нужно получить число без его знака? Например, получить 3, если имеется -3.
 * Для его мы можем использовать функцию модуля. Модуль числа это его часть без знака.
 * Модуль положительно числа равен ему самому.
 *
 * В Java модуль можно получить с использованием функции Math.abs. Синтаксис такой
 *
 * double переменная = Math.abs(число);
 *
 * Например,
 *
 * double result = Math.abs(-3); // = 3
 *
 * Ниже приведен код. Вам нужно его раскоментировать и получить модуль -5
 */
